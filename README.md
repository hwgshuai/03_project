...
设计并实现了独立的 LabelVersion（标签版本表），彻底改变了传统覆盖式修改的弊端。

防篡改机制： 实现了基于 SHA-256 的哈希值自动计算，确保任何微小的标签改动都能被系统识别并记录。

落地了“双重校验”风控流程：

在 ShipmentBatch（出库批次表）中植入了严格的状态机逻辑。
...
### WarehouseLocation API
- `GET /api/locations/` - 获取库位列表
- `POST /api/locations/` - 创建新库位
- `GET /api/locations/{id}/` - 获取特定库位
- `PUT /api/locations/{id}/` - 更新库位
- `DELETE /api/locations/{id}/` - 删除库位

### InventoryStock API
- `GET /api/inventory/` - 获取库存列表
- `GET /api/inventory/{id}/` - 获取特定库存记录

### InboundReceipt API
- `GET /api/receipts/` - 获取入库单列表
- `POST /api/receipts/` - 创建新入库单
- `GET /api/receipts/{id}/` - 获取特定入库单
- `PUT /api/receipts/{id}/` - 更新入库单
- `DELETE /api/receipts/{id}/` - 删除入库单

### InboundLineItem API
- `POST /api/receipts/{receipt_id}/items/` - 为指定入库单添加明细项
- `PUT /api/items/{item_id}/` - 更新入库明细项

### OutboundExecution API
- `GET /api/executions/` - 获取出库任务列表
- `POST /api/executions/` - 创建新出库任务
- `GET /api/executions/{id}/` - 获取特定出库任务
- `PUT /api/executions/{id}/` - 更新出库任务
- `DELETE /api/executions/{id}/` - 删除出库任务

### StockTransaction API
- `GET /api/transactions/` - 获取库存流水列表
- `GET /api/transactions/{id}/` - 获取特定库存流水记录
